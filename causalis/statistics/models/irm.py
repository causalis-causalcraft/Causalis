...
class IRMResults:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "coefficient": float(self.coef[0]) if self.coef.size > 0 else None,
            "stderr": float(self.se[0]) if self.se.size > 0 else None,
...
class IRM(BaseEstimator):
    ...
    def effect(self, score: Optional[str] = None, confidence_level: float = 0.95) -> IRMResults | np.ndarray:
        ...
        Returns
        -------
        IRMResults or np.ndarray
            For ATE/ATTE, returns an IRMResults container.
        ...
        # psi elements
        if score == "ATTE" and self.normalize_ipw:
        ...
        psi_a = -w / (w_mean if w_mean != 0.0 else 1.0)  # ensures E[psi_a] = -1

        theta_hat = float(np.mean(psi_b))
        ...
        # Update internal state if calculating default score
        if score == self.score:
            self.psi_a_ = psi_a
            self.psi_b_ = psi_b
            self.psi_ = psi
            self.coef_ = results.coef
            self.se_ = results.se
    ...
    def gate(self, groups: pd.DataFrame | pd.Series, level: float = 0.95) -> BLP:
        ...
        # We use the existing BLP class from causalis.statistics.models.blp
        blp_model = BLP(
...
def irm_dml(data: CausalData, **kwargs) -> Dict[str, Any]:
    ...
    data : CausalData
        Data container.
    **kwargs : dict
        Arguments passed to IRM constructor.
    ...
    if isinstance(res, IRMResults):
        ...
        # For CATE (orthogonal signals), return as a list in the dict
        inference = {"cate": res.tolist() if hasattr(res, "tolist") else res}
